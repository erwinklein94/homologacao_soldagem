...
insert into public.provas (codigo, titulo, descricao, nota_minima) values ('A', 'Prova A — Fundamentos, Preparação e Segurança', 'Fundamentos do procedimento, requisitos de equipe, ferramentas e segurança na soldagem aluminotérmica de trilhos.', 7) on conflict (codigo) do nothing;
insert into public.provas (codigo, titulo, descricao, nota_minima) values ('B', 'Prova B — Execução, Montagem e Parâmetros', 'Tolerâncias de montagem, preparação da junta, parâmetros de corte, nivelamento e pressão dos gases.', 7) on conflict (codigo) do nothing;
insert into public.provas (codigo, titulo, descricao, nota_minima) values ('C', 'Prova C — Acabamento, Qualidade e Tempos', 'Tempos de processo, acabamento, esmerilhamento, tagueamento, registro no SIV e liberação da via.', 7) on conflict (codigo) do nothing;
...
from public.provas p where p.codigo = 'A'
...
from public.provas p where p.codigo = 'B'
on conflict (prova_id, ordem) do nothing;
insert into public.questoes (prova_id, ordem, enunciado, alternativas, correta, justificativa)
...
from public.provas p where p.codigo = 'C'
on conflict (prova_id, ordem) do nothing;
insert into public.questoes (prova_id, ordem, enunciado, alternativas, correta, justificativa)
